...
class FastDVDnet(BaseNet):
	...
	def __init__(self, num_input_frames=5):
		super(FastDVDnet, self).__init__('fastdvdnet')
	...
	def forward(self, input_dict, device):
		...
		noise_map = input_dict['noise_map'].to(device)
	...
	def load_pre_trained(self, dir_path):
		checkpoint_path = os.path.join(dir_path, 'fastdvdnet.pth')
